...
async def run_test():
    ...
    try:
        ...
        # -> Open the invoice editor by clicking the 'Start Building Your Invoice' button to continue without signing in.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/section/div[4]/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the first line item description with 'Item with invalid amount', set its amount field to non-numeric text, then click Export PDF to observe validation behavior.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main[2]/aside/div[5]/div/div/div[2]/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Item with invalid amount')
